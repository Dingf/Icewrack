modifier_iw_lina_warmth_caster = class({})

function modifier_iw_lina_warmth_caster:IsAura()
...
function modifier_iw_lina_warmth_caster:GetModifierAura()
	return "modifier_iw_lina_warmth"
end

function modifier_iw_lina_warmth_caster:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY + DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_iw_lina_warmth_caster:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end

function modifier_iw_lina_warmth_caster:GetAuraRadius()
	return self:GetAbility():GetAOERadius()
end

function modifier_iw_lina_warmth_caster:GetAuraEntityReject(hEntity)
	return not hEntity:IsAlive() or hEntity == self:GetCaster()
end

function modifier_iw_lina_warmth_caster:IsAuraActiveOnDeath()
	return false
end

function modifier_iw_lina_warmth_caster:OnCreated(args)
	if IsServer() then
